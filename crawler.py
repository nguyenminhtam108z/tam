...
class CrawlerBaoNinhBinh(BaseCrawler):
    MAX_PAGE = 3
    SMY = Dmysql(host=mysql_config['host'], user=mysql_config['user'], passwd=mysql_config['passwd'], db=mysql_config['db'], keep_connect=True)
    ...
    def upsert_category(self, category):
        category_from_db = self.SMY.query_row('SELECT id FROM category WHERE id=:id LIMIT 1', {'id': category['id']})
